...
using namespace firstSpace;// разрешили использовать в глобальном пространстве
namespace firstSpace {
    int  a = 1; //одно и то же что и в одном namespace
}
...
class Point <string> {
public:
    Point(string x, string y) { this->x = x; this->y = y; }
    void show() {
        cout << "тип x: " << typeid(x).name() << endl;
        cout << "тип y: " << typeid(y).name() << endl;
        cout << x.c_str() << endl;
...
template <>
class Point3D<int> : public Point<int> {
public:
